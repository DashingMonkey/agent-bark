...
pub enum EventKind {
    ...
    /// 失败色全屏特效 + 事件流红色「任务失败」+ 系统通知 + 会话被摘出运行中表，
    /// 而下一秒重试就成功了，全是噪音。归一成这一类后：会话当心跳留在表里
    /// （相位回落思考中）、不通知、不触发失败色；事件仍入历史与事件流
    ...
    /// （新回合才有），不收「工具收尾」的话会话相位会一直卡在等待中、等待色一直亮到
    ...
    /// 而「Stop 之后紧跟的终态回声」又会把正常完成的完成色压掉。按失败处理会亮失败色 +
    /// 弹「任务失败」（用户自己按的，纯噪音），按完成处理会亮完成色（谎报完成）。
    /// 统一成这一类：**不通知**；流光在没有别的会话在跑时**直接收起**（不亮失败色也不亮完成色），
    /// 还有别的会话在跑就照常显示它们的状态。
    RunAborted,
}
